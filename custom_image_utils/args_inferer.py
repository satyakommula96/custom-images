...
def _get_dataproc_version_from_image_family(image_family_uri):
    ...
    project, image_family_name = _extract_image_name_and_project_from_family_uri(
        image_family_uri
    )
    command = [
        "gcloud",
    ...
    with tempfile.NamedTemporaryFile() as temp_file:
        ...
        # parse the first ready image with the dataproc version attached in labels
        if stdout:
            dataproc_version = stdout.decode(
                "utf-8"
            ).strip()  # should be just one value
...
def _extract_image_family_path(image_family_uri):
    ...
    project, image_name = _extract_image_name_and_project_from_family_uri(
        image_family_uri
    )
...
def _get_dataproc_image_path_by_version(version):
    ...
    if len(parsed_version) == 2:
        ...
        filter_arg = (
            "labels.goog-dataproc-version ~ ^{}-{}$ AND NOT name ~ -eap$"
            " AND status = READY"
        ).format(parsed_version[0], parsed_version[1])
    else:
        major_version = parsed_version[0]
        ...
        filter_arg = (
            "labels.goog-dataproc-version = {}-{}-{} AND NOT name ~ -eap$"
            " AND status = READY"
        ).format(parsed_version[0], parsed_version[1], parsed_version[2])
    command = [
        "gcloud",
        "compute",
        "images",
    ...
    with tempfile.NamedTemporaryFile() as temp_file:
        ...
        if stdout:
            ...
            for line in parsed_lines:
                ...
                if len(parsed_image) == 2:
                    ...
                    if parsed_image_version not in all_images_for_version:
                        all_images_for_version[parsed_image_version] = [
                            _IMAGE_PATH.format("cloud-dataproc", parsed_image_name)
                        ]
                        image_versions.append(parsed_image_version)
                    else:
                        all_images_for_version[parsed_image_version].append(
                            _IMAGE_PATH.format("cloud-dataproc", parsed_image_name)
                        )
            ...
            _LOG.info(
                "Choosing image %s with version %s",
                all_images_for_version[image_versions[0]][0],
                image_versions[0],
            )
...
def _infer_base_image(args):
    ...
    if args.base_image_uri:
        ...
        args.dataproc_base_image, args.dataproc_version = (
            _get_dataproc_image_path_by_version(args.dataproc_version)
        )
    elif args.base_image_family:
        args.dataproc_base_image = _extract_image_family_path(args.base_image_family)
        args.dataproc_version = _get_dataproc_version_from_image_family(
            args.base_image_family
        )
    else:
        raise RuntimeError(
...
def _infer_oauth(args):
    if args.oauth:
        args.oauth = '\n    "OAuthPath": "{}",'.format(os.path.abspath(args.oauth))
    else:
        args.oauth = ""
