...
_FULL_IMAGE_URI = re.compile(
    r"^(https://www\.googleapis\.com/compute/([^/]+)/)?projects/([^/]+)/global/images/([^/]+)$"
)
_FULL_IMAGE_FAMILY_URI = re.compile(
    r"^(https://www\.googleapis\.com/compute/([^/]+)/)?projects/([^/]+)/global/images/family/([^/]+)$"
)
_LATEST_FROM_MINOR_VERSION = re.compile(
    r"^(\d+)\.(\d+)-(?:[a-z0-9\-]+-)?((?:debian|ubuntu|rocky)\d+)$"
)
_ARM_ARCH_REGEX = re.compile(
    r"""
...
def parse_args(args):
    ...
    image_args.add_argument(
        "--dataproc-version", type=_version_regex_type, help=constants.version_help_text
    )
    image_args.add_argument(
        "--base-image-uri",
    ...
    parser.add_argument(
        ...
        required=False,
        default="default",
        help="""(Optional) The service account that is used to launch the VM instance
    ...
    parser.add_argument(
        ...
        required=False,
        default={},
        help="""(Optional) Additional files/directories uploaded along with
    ...
    parser.add_argument(
        ...
        required=False,
        default=30,
        help="""(Optional) The size in GB of the disk attached to the VM instance
    ...
    parser.add_argument(
        ...
        required=False,
        default=None,
        help="""(Optional) The accelerators (e.g. GPUs) attached to the VM instance
    ...
    parser.add_argument(
        ...
        required=False,
        default=None,
        help="""(Optional) The storage location (e.g. US, us-central1) of the custom
    ...
    parser.add_argument(
        ...
        required=False,
        default=300,
        help="""(Optional) The time to wait in seconds before shutting down the VM
    ...
    parser.add_argument(
        ...
        choices=["cli", "api"],
        default="cli",
        help="""(Optional) The execution engine used to create the image.
      Defaults to 'cli' (gcloud / gsutil command line tools).
      Use 'api' to use native Google Cloud client libraries.
      """,
    )
    ...
    if parsed_args.machine_type is None:
        is_arm = (
            parsed_args.base_image_uri
            and _ARM_ARCH_REGEX.search(parsed_args.base_image_uri)
        ) or (
            parsed_args.base_image_family
            and _ARM_ARCH_REGEX.search(parsed_args.base_image_family)
        )
        if is_arm:
