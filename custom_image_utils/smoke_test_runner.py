...
def _create_workflow_template(
    workflow_name, image_name, project_id, zone, region, network, subnet, no_external_ip
):
    ...
    if pipe.returncode != 0:
        raise RuntimeError(
            "Error creating Dataproc workflow template '%s'.", workflow_name
        )
    ...
    if pipe.returncode != 0:
        raise RuntimeError(
            "Error setting cluster for Dataproc workflow template '%s'.", workflow_name
        )
    ...
    if pipe.returncode != 0:
        raise RuntimeError(
            "Error adding job to Dataproc workflow template '%s'.", workflow_name
        )
...
def _verify_custom_image(
    image_name, project_id, zone, network, subnetwork, no_external_ip
):
    ...
    try:
        _LOG.info(
            "Creating Dataproc workflow-template %s with image %s...",
            workflow_name,
            image_name,
        )
        ...
        _LOG.info(
            "Successfully smoke tested Dataproc workflow-template %s...", workflow_name
        )
        ...
        try:
            ...
            _LOG.info(
                "Successfully deleted Dataproc workflow-template %s...", workflow_name
            )
        except RuntimeError:
            pass
